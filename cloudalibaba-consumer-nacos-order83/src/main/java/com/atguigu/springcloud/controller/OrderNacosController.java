...
//    @SentinelResource(value = "fallback") //没有配置
  ...
  @SentinelResource(value = "fallback",fallback = "handlerFallback", blockHandler = "blockHandler",
    ...
    public CommonResult<Payment> fallback(@PathVariable("id") Long id){
        CommonResult<Payment> commonResult = restTemplate.getForObject(SERVICE_URL + "/payment/nacos/" + id, CommonResult.class);
        if(id == 4){
...
//------ openfeign--------------------------------------------
    ...
    public CommonResult paymentInfo(@PathVariable("id") Long id){
        CommonResult payment = paymentService.getPayment(id);
        if(id == 4){
            throw new IllegalArgumentException("IllegalArgumentException,非法参数异常");
